...
def is_within(child: str, base: str) -> bool:
    """
    判断 child 路径是否位于 base 目录内（含 base 本身）。
    用于工具层的越权校验：resolve_path 负责解析，is_within 负责兜底防穿越。
    """
    try:
        child_path = Path(child).resolve()
        base_path = Path(base).resolve()
        return child_path == base_path or base_path in child_path.parents
    except Exception:
        return False