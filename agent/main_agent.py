...
# ---------------------------- 会话历史持久化（Checkpointer） ----------------------------
# 默认使用 AsyncSqliteSaver（异步环境适配版）：会话历史落盘到 data/checkpoints.db，
# 服务重启后同一 thread_id 仍可续聊；若未安装 langgraph-checkpoint-sqlite 则降级为内存保存。
# 注意1：原 InMemorySaver 在 FastAPI 异步环境下重启即丢，且无法跨进程共享。
# 注意2：AsyncSqliteSaver 构造时要求"运行中的事件循环"（内部要绑定 loop），
#        因此 Agent 的构建被延迟到第一次任务执行时（此时已处于事件循环内），
#        通过 _get_main_agent() 惰性单例实现。
_db_path = project_root_path / "data" / "checkpoints.db"
_main_agent = None  # 惰性单例缓存
_main_agent_lock = asyncio.Lock()  # 并发首次请求时防止重复构建 graph / sqlite 连接

async def _get_main_agent():
    """惰性构建主智能体（必须在事件循环内首次调用）"""
    global _main_agent
    if _main_agent is not None:
        return _main_agent

    async with _main_agent_lock:
        # 双重检查：拿到锁前可能已被并发请求构建完成
        if _main_agent is not None:
            return _main_agent
        ...
        try:
            ...
            _sqlite_conn = aiosqlite.connect(str(_db_path))
            checkpointer = AsyncSqliteSaver(_sqlite_conn)
            logger.info("会话历史将持久化到 %s", _db_path)
        except ImportError:
            # 依赖缺失时优雅降级（与 RAGFlow/MySQL 可选依赖的设计保持一致）
            from langgraph.checkpoint.memory import InMemorySaver
            checkpointer = InMemorySaver()
            logger.warning("未安装 aiosqlite / langgraph-checkpoint-sqlite，会话历史降级为内存保存（重启丢失）。"
                           "如需持久化请执行: pip install langgraph-checkpoint-sqlite")
...
async def run_deep_agent(task_query,session_id):
    ...
    """
    logger.info("当前会话的main_agent开始执行了！ 会话id:%s", session_id)
    ...
    # 反馈结果
    try:
        # 惰性构建主智能体（首次调用时在事件循环内创建 AsyncSqliteSaver 和 graph）
        main_agent = await _get_main_agent()
        # 执行
                ...
                if messages and isinstance(messages,list):
                    ...
                    if node_name == 'model':
                        if last_msg.tool_calls:
                            ...
                            # 最终结果
                            logger.info("主智能体执行结果，最终结果：%s", str(last_msg.content)[:100])
        ...
        if "Content Exists Risk" in err_str:
            # DeepSeek 内容风控：提问或中间内容（如搜索到的网页原文）触发了服务商审核
            monitor.report_error("模型服务商（DeepSeek）内容审核未通过：本次提问或搜索到的中间内容包含敏感词。"
                                 "请换一种表述重新提问（避免敏感关键词），或换个主题再试。")
        else:
            monitor.report_error(f"执行主智能发生异常信息：{err_str}")
    finally:
        # 释放存储的地址和session_id
