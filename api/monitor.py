...
# 全局运行时（用于脚本模式下的流式输出；FastAPI 模式下不存在，hasattr 判断兜底）
import builtins
    ...
    def _emit(self, event_type: str, message: str, data: Optional[Dict[str, Any]] = None):
        ...
        if self.websocket_manager:
            try:
                ...
                if manager_loop:
                    if thread_id:
                        ...
                        try:
                            ...
                            logger.debug("对比是不是同一个event_loop:%s", manager_loop == current_loop)
                        except RuntimeError:
                            current_loop = None
                ...
                logger.warning("[Monitor] WebSocket send failed: %s", e)
        ...
        # 3. 日志保底输出 (方便调试)
        if event_type == "error":
            logger.error("[Monitor:%s] %s", event_type, message)
        else:
...
class ConnectionManager:
    def __init__(self):
        # 同一 thread_id 允许多个连接（多标签页场景），避免互相覆盖/误删
    ...
    def set_loop(self, loop):
        ...
        logger.info("[Monitor] ConnectionManager manually bound to loop: %s", id(self.loop))

    async def connect(self, websocket: WebSocket, thread_id: str):
        await websocket.accept()
        logger.info("存储当前会话id:%s对应的连接", thread_id)
        self.active_connections.setdefault(thread_id, []).append(websocket)
        logger.info("Client connected: %s (当前连接数: %d)", thread_id, len(self.active_connections[thread_id]))
    ...
    async def send_to_thread(self, message: dict, thread_id: str):
        # 向该会话的所有连接广播；发送失败的连接顺手清理
        for websocket in list(self.active_connections.get(thread_id, [])):
            try:
